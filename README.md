...
AUC was identical before and after scaling in all 30 fold-runs, as monotonicity
requires — a useful internal check that the implementation is correct.
...
- **T is fitted on 37 validation subjects**, which is small enough to overfit.
  ...
  0.544 to 4.070; mean clinical ECE worsened (0.1905 → 0.2424). Temperature
  scaling assumes a validation set large enough to estimate one parameter
  stably, and at this cohort size that assumption is marginal.
