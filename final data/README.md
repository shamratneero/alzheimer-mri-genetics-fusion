...
On OASIS-3 (365 subjects, 5-fold cross-validation), **the conventional way of
selecting and reporting a model manufactures apparent instability, and a
one-line change largely removes it.**

Under the standard rule — keep the checkpoint with the highest validation AUC —
...
Ranked by mean per-fold AUC — the near-universal convention — fusion wins.
Ranked by pooled out-of-fold AUC over the *same predictions*, fusion is
significantly **worst** (vs clinical ΔAUC 0.094, p=0.003; vs imaging ΔAUC 0.059,
p=0.039; paired bootstrap on identical subjects).

The natural reading is that multimodal fusion fails at this scale. **That
reading is wrong.** Re-selecting the checkpoint from the *same training runs*
using a calibration-aware criterion gives:

| mode | pooled AUC (max val AUC) | pooled AUC (min val Brier) | Δ | p |
|---|---|---|---|---|
| clinical | 0.7638 | **0.7983** | +0.034 | <0.001 |
| imaging | 0.7292 | **0.8392** | +0.110 | <0.001 |
| fusion | 0.6702 | **0.8195** | +0.149 | <0.001 |

The per-fold-to-pooled gap — the quantity that made fusion look broken — falls
from 0.199 to 0.041. Degenerate folds go from one per mode to zero. Same
weights, same folds, same training: only the epoch chosen differs.

**The instability was largely an artefact of checkpoint selection, not a
property of multimodal fusion.**
...
decision threshold. Fusion fold 4 makes the point in a single table — one
training run, four checkpoints, chosen by four different rules:

| selection rule | epoch | test AUC | balanced acc | ECE | Brier |
|---|---|---|---|---|---|
| max validation AUC | 10 | **0.897** | **0.500** | **0.486** | **0.474** |
| max (AUC − 0.5·ECE) | 5 | 0.855 | 0.739 | 0.119 | 0.172 |
| max AUC given bacc>0.55 | 5 | 0.855 | 0.739 | 0.119 | 0.172 |
| min Brier | 17 | 0.860 | 0.729 | 0.138 | 0.174 |

The checkpoint chosen by the standard rule has the **highest AUC of the four and
is unusable**: balanced accuracy exactly 0.500, meaning it predicted one class
for all 73 test subjects. Conceding 0.042 AUC yields a working model with a
four-fold lower calibration error.

This is not the criterion failing by its own standard — it is the standard being
wrong. Reported as most papers report, AUC and accuracy on a single split, the
degenerate model would have looked like the best in the study.

Across 10 imaging and fusion fold-runs, AUC-based selection chose an epoch whose
validation balanced accuracy had **already collapsed to ~0.500** in four of
them, while a better-behaved epoch existed in the same run.
...
# checkpoint-selection experiment: four criteria tracked in one run per mode
...
Known limitations: n=365 caps absolute performance; the four selection criteria
were compared on the same test folds rather than chosen on a nested inner loop,
so criterion choice is not itself cross-validated; the λ=0.5 weight in the
AUC−ECE criterion is arbitrary and untuned (and is the worst of the three
alternatives); OASIS-3 labels derive from CDR whereas ADNI/AIBL use
physician-assigned NINCDS-ADRDA criteria, so cross-cohort label equivalence is
not exact.
